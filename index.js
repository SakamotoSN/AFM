...
if(message.content === "!debug"){
  let split = JSON.stringify(text).split(`","`);
  var ID = JSON.stringify(split[0]).slice('16','-1') 
  var auteur = JSON.stringify(split[1]).slice('10','-1')
  var date = JSON.stringify(split[2]).slice('9','-1')
  var mess = JSON.stringify(split[3]).slice('12','-1')
  var image = JSON.stringify(split[4]).slice('16','-1')
